...
$$p(\boldsymbol{y|\beta})=exp\{\boldsymbol{y}^T\boldsymbol{X\beta-I}_n^Texp(\boldsymbol{X\beta})-\boldsymbol{I}^T_nlog(\boldsymbol{y}!)\}$$

- This leads to an integral that has no closed form solution (intractable):

$$
\begin{array}{l}
p(\boldsymbol{y})= (2\pi)^{-(k+1)/2}|\boldsymbol{\Sigma_\beta}|^{-1/2}\\*\int_{\mathbb{R}^{k+1}}exp\{\boldsymbol{y}^T\boldsymbol{X\beta}-\boldsymbol{I}_n^T exp(\boldsymbol{X\beta})-\boldsymbol{I}^T_nlog(\boldsymbol{y}!)
...
&+\frac{1}{2} \log \left|\boldsymbol{\Sigma}_{q(\boldsymbol{\beta})}\right|-\frac{1}{2} \log \left|\boldsymbol{\Sigma}_{\boldsymbol{\beta}}\right|+\frac{k+1}{2} \\
&-\mathbf{1}_{n}^{T} \log (\mathbf{y} !)
\end{aligned}
$$
...
- From earlier,
$$
\log p(y) \geq \log \underline{p}(\boldsymbol{y; \mu_{q(\beta)}, \Sigma_{q(\beta)}})
$$
...
$$
\begin{array}{l}
p(y|\beta) = exp[\boldsymbol{y}^T\boldsymbol{X\beta -I_n^T}log(\boldsymbol{I_n}+exp(\boldsymbol{X\beta}))
\\-\frac{1}{2}(\boldsymbol{\beta-\mu_\beta})^T\boldsymbol{\Sigma^{-1}_\beta(\beta-\mu_\beta)
...
- Doesn't cover the accuracy of the approximations
  - makes it hard to compare with MCMC
