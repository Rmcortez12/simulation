...
knitr::kable(a,format = "html", table.attr = "style='width:60%;'")
```
...
a <- data.frame("Exact CI"  = ex, "Asymptotic CI" = as, "Bootstrapped CI" = bt,row.names = c("lower","upper"))
knitr::kable(a,format = "html", table.attr = "style='width:60%;'")
```
The Exact confidence interval has the smallest width, which makes sense since it has the strongest assumptions, followed by the confidence interval that was estimated with bootstrap, and the asymptotic confidence interval was just barely larger than the latter. 
...
# Percentile
Boot.t <- unname(quantile(Tc.npboot, probs = c(alpha/2, 1-alpha/2)))   # Bootstrap percentile CI of P(X>100)
...
all <- data.frame("Bootstrap-t" = Boot.t, "BCa" = BCa, row.names = c("Lower", "Upper"))

knitr::kable(all,format = "html", table.attr = "style='width:60%;'")
```

Interestingly enough the width of both intervals is the same, the only difference between the 2 seems to be where they are centered.The BCa which adjust for Bias seems to have shifted closer to 0.  
